...
package com.flashgangsta.managers {
	...
	public class MappingManager {
		...
		/**
		 * Пропорционально скалирует объект так, что бы он не выхадил за границы указанной зоны
		 * @param	target объект подвергаемый скалированию
		 * @param	area зона
		 */
		
		static public function setScaleByArea( target:DisplayObject, area:Rectangle ):void {
			var targetRect:Rectangle = target.getBounds( target );
			target.scaleX = target.scaleY = Math.min( area.width / targetRect.width, area.height / targetRect.height );
			roundObjectSides( target, Math.floor );
		}
